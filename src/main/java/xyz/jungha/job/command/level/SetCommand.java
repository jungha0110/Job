...
public class SetCommand extends LevelSubCommand {
    ...
    protected void applyChange(OfflinePlayer player, Jobs job, int amount) {
        jobService.setJobLevel(player, job, amount);
    }

    @Override
    protected String getSuccessMessage(OfflinePlayer player, Jobs job, int amount) {
        return String.format("[<gold>직업<white>] <green>%s</green>님의 <yellow>%s</yellow> 직업 레벨을 <gold>%d</gold>로 설정했습니다.", player.getName(), job.getDisplayName(), amount);
    }
}
